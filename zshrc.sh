...
export __GIT_PROMPT_DIR=${__GIT_PROMPT_DIR:-$(dirname $0)}
# Initialize colors.
autoload -U colors
colors
...
git_super_status() {
    ...
    if [ -n "$__CURRENT_GIT_STATUS" ]; then
	  ...
	  if [ -n "$GIT_REMOTE" ]; then
		  STATUS+="$ZSH_THEME_GIT_PROMPT_REMOTE$GIT_REMOTE%{${reset_color}%}"
	  fi
	  ...
	  if [ "$GIT_UNTRACKED" -ne "0" ]; then
		  STATUS+="$ZSH_THEME_GIT_PROMPT_UNTRACKED%{${reset_color}%}"
	  fi
